...
Web Tracks release — the repository now ships a fully self-contained browser
groovebox alongside the Python library.

### Added

- **Web Tracks** (`webapp/`) — an unofficial, fan-made browser groovebox
  inspired by the Circuit Tracks. 100% static and self-contained: 8 tracks
  (2 synth, 2 MIDI, 4 drum), 4×8 pad grid, Web Audio synth engine, and the
  hardware's view system (Note, Velocity, Gate, Probability, Micro Step,
  Pattern Settings, Patterns, Mixer, FX, Side Chain, Scales).
  ...
  - **First-run welcome overlay** with quick start + disclaimer, keyboard
    mapping overlay, music-reactive glow around the console, SVG favicon
  - **Deployment** — `webapp/Dockerfile` (nginx) and Coolify instructions
- `scripts/backup_projects.py` — byte-perfect `.ncs` backups of device
  project slots over SysEx

### Changed

- The webapp carries its own identity — **Web Tracks** wordmark, violet-slate
  palette, and "not affiliated with Novation/Focusrite" disclaimers in the
  app, both READMEs, and the welcome overlay. All bundled audio content is
  generated; nothing is copied from factory packs.

[0.3.0]: https://github.com/namirsab/circuit-tracks-tools/compare/v0.2.0...v0.3.0
