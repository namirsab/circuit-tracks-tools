...
class SynthStep:
    ...
    notes: list[NCSNote] = field(default_factory=lambda: [NCSNote() for _ in range(NOTES_PER_STEP)])
    ...
    def active_notes(self) -> list[NCSNote]:
        return [self.notes[i] for i in range(NOTES_PER_STEP) if self.assigned_note_mask & (1 << i)]
    ...
    def from_bytes(cls, data: bytes) -> SynthStep:
        notes = [NCSNote.from_bytes(data[4 + i * 4 : 4 + (i + 1) * 4]) for i in range(NOTES_PER_STEP)]
        return cls(
            assigned_note_mask=data[0],
            probability=data[1],
            reserved=data[2:4],
            notes=notes,
        )
...
class PatternSettings:
    ...
    def to_bytes(self) -> bytes:
        return bytes([self.playback_end, self.playback_start, self.sync_rate, self.playback_direction]) + self.reserved

    @classmethod
    def from_bytes(cls, data: bytes) -> PatternSettings:
        return cls(
            playback_end=data[0],
            playback_start=data[1],
            sync_rate=data[2],
            playback_direction=data[3],
            reserved=data[4:40],
        )
...
class SynthPattern:
    ...
    steps: list[SynthStep] = field(default_factory=lambda: [SynthStep() for _ in range(STEPS_PER_PATTERN)])
...
class DrumPattern:
    ...
    steps: list[DrumStep] = field(default_factory=lambda: [DrumStep() for _ in range(STEPS_PER_PATTERN)])
    settings: PatternSettings = field(default_factory=PatternSettings)
    raw_header: bytes = b""
    pre_data: bytes = b""
    post_data: bytes = b""
...
class Scene:
    ...
    track_chains: list[ChainEntry] = field(default_factory=lambda: [ChainEntry() for _ in range(NUM_TRACKS)])
...
class NCSFile:
    ...
    pattern_chains: list[ChainEntry] = field(default_factory=lambda: [ChainEntry() for _ in range(NUM_TRACKS)])
...
def parse_ncs(path: str | Path) -> NCSFile:
    ...
    # Scenes
    ncs.scenes = [
        Scene.from_bytes(data[sc_base + i * _SCENE_SIZE : sc_base + (i + 1) * _SCENE_SIZE]) for i in range(NUM_SCENES)
    ]
    ...
    ncs.drum_configs = [
        DrumTrackConfig.from_bytes(
            t[_DRUM_CONFIGS_OFFSET + i * _DRUM_CONFIG_SIZE : _DRUM_CONFIGS_OFFSET + (i + 1) * _DRUM_CONFIG_SIZE]
        )
...
def _write_automation_to_blocks(ncs: NCSFile) -> None:
    ...
    for block_idx, pattern in block_patterns.items():
        has_locks = (isinstance(pattern, DrumPattern) and pattern.param_locks) or (
