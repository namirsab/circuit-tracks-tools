...
class TestInstructions:
    def test_instructions_loaded(self):
        ...
        assert "Circuit Tracks" in instructions
        assert "connect" in instructions

    def test_instructions_file_exists(self):
        from pathlib import Path
        content_dir = Path(__file__).parent.parent / "src" / "circuit_mcp" / "content"
        assert (content_dir / "instructions.md").is_file()


class TestResources:
    def test_genre_recipes_content(self):
        import circuit_mcp.server as srv
        content = srv.genre_recipes()
        assert isinstance(content, str)
        assert len(content) > 100
        assert "Techno" in content
        assert "BPM" in content

    def test_song_format_content(self):
        import circuit_mcp.server as srv
        content = srv.song_format_reference()
        assert isinstance(content, str)
        assert len(content) > 100
        assert "load_song" in content
        assert "patterns" in content


class TestPrompts:
    def test_create_track_default(self):
        import circuit_mcp.server as srv
        messages = srv.create_track()
    ...
    def test_create_track_custom(self):
        import circuit_mcp.server as srv
        messages = srv.create_track(genre="house", mood="uplifting", bpm=124)
        text = messages[0].content.text
        assert "house" in text
        assert "uplifting" in text
        assert "124" in text

    def test_design_sound_default(self):
        import circuit_mcp.server as srv
        messages = srv.design_sound()
        text = messages[0].content.text
        assert "synth 1" in text
        assert "warm evolving pad" in text

    def test_design_sound_custom(self):
        import circuit_mcp.server as srv
        messages = srv.design_sound(synth=2, description="aggressive acid bass")
        text = messages[0].content.text
        assert "synth 2" in text
        assert "aggressive acid bass" in text

    def test_modify_project_default(self):
        import circuit_mcp.server as srv
        messages = srv.modify_project()
        text = messages[0].content.text
        assert "slot 0" in text

    def test_modify_project_with_description(self):
        import circuit_mcp.server as srv
        messages = srv.modify_project(slot=3, description="make it faster")
        text = messages[0].content.text
        assert "slot 3" in text
        assert "make it faster" in text

    def test_live_performance(self):
        import circuit_mcp.server as srv
        messages = srv.live_performance()
        text = messages[0].content.text
        assert "live" in text.lower() or "performance" in text.lower()
        assert "morph" in text.lower()
