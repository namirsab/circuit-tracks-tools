...
class TestPatchBuilder:
    ...
    def test_filter(self):
        patch = PatchBuilder().filter(
            frequency=80, resonance=40, filter_type=3, drive=50
        ).build()
...
class TestMacros:
    def test_set_macro_by_name(self):
        patch = PatchBuilder().set_macro(1, [
            {"dest": "filter_frequency", "start": 0, "end": 127, "depth": 64}
        ]).build()
    ...
    def test_macro_8_address(self):
        ...
        patch = PatchBuilder().set_macro(8, [
            {"dest": "chorus_level", "start": 0, "end": 127}
        ], position=50).build()
